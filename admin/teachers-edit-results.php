		...
		<div class="contents">
			...
			<center class="white_background">
				<div class="edit_r_table">
					...
					<table id="datatable-buttons" style="display: ;" class="table table-bordered">
						<thead>
							<tr>
								<th>
									Teacher ID
	...
	(function() {
		$(document).ready(function() {
			$(document).on('blur','tr.editable td',function() {
				...
				// alert(value)
				$.post('../portal/ajax/misc.php',{type:type,r_Id:r_Id,dValue:dValue},function(data) {
			...
			$(document).on('click','#insertStudent',function() {
				...
				$.post('../portal/ajax/misc.php',{t_id:t_id,	st_id:st_id,  sub_id:sub_id, 	cla_id:cla_id,
			...
			$(document).on('click','.removeId',function() {
				...
				if (confirm("Remove this student's Result?")) {
					if (confirm('Sure?')) {
						$.post('../portal/ajax/misc.php',{removeId:removeId},function(data) {
							$('#error_feed').fadeIn(100).text(data).delay(2000).fadeOut(50);
						});
					}
				}
